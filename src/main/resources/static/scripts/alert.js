(function($) {        
  ...
  var GenerateCss = function () {  
    ...
    $("#mb_con").css({ zIndex: '999999', width: '350px',height:'50%', position: 'fixed',
    ...
    $("#origpass").css('margin','20px');
    $("#newpass").css('margin','20px');
    $("#newpass").css('margin-bottom','0px');
    $("#origpass input").bind("change",function () {
        origpass = $("#origpass input").val();
    });
    $("#newpass input").bind("change",function () {
        newpass = $("#newpass input").val();
    });
    $("#mb_msg").css({ padding: '20px', margin: '3px',
      fontSize: '14px' ,color:'#4c4c4c'
    });  
   
    $("#mb_ico").css({ display: 'block', position: 'absolute', right: '10px', top: '9px',  
      border: '1px solid Gray', width: '18px', height: '18px', textAlign: 'center',  
      cursor: 'pointer', borderRadius: '12px', fontFamily: '微软雅黑'  
    });  
    ...
    //让提示框居中  
    $("#mb_con").css({ top:"15%", left: "calc(50% - 175px)" });
